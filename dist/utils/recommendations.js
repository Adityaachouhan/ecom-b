 ...
 */
export function coOccurringProductIds(orders, productId, limit) {
 ...
 */
export function getRelatedProducts(products, orders, product, limit = 4) {
    const byId = new Map(products.map((p) => [p.id, p]));
    const coOccurring = coOccurringProductIds(orders, product.id, limit)
        .map((id) => byId.get(id))
        .filter((p) => Boolean(p));
    if (coOccurring.length >= limit)
