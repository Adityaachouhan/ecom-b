...
export async function ensureNotificationTemplates() {
    ...
    if (!db.notificationTemplates.length) {
        ...
        for (const t of defaults) {
            const exists = db.notificationTemplates.find((x) => x.eventType === t.eventType && x.channel === t.channel);
            if (!exists)
                db.notificationTemplates.push(t);
        }
    }
...
export function hasNotificationLog(eventType, refId) {
 ...
 */
export async function sendNotification(eventType, userId, data = {}) {
    if (!userId)
        return;
...
export async function checkReviewReminders() {
    ...
    const due = db.orders.filter((o) => {
        if (o.status !== 'delivered' || !o.deliveredAt)
            return false;
        if (now - new Date(o.deliveredAt).getTime() < REVIEW_MS)
            return false;
...
export function startNotificationScheduler() {
    setInterval(() => {
        void checkReviewReminders().catch((err) => console.error('Review reminder check failed:', err));
    }, 30_000);
}
