...
import { ALL_ROLES, authenticate, requireSuperAdminOrigin, signToken, } from '../middleware/auth.js';
import { fail, generateId, success, todayISO } from '../utils/helpers.js';
const router = Router();
function toAuthUser(account, extras) {
    return {
        id: account.id,
        email: account.email,
        name: account.name,
...
function resolveSuperAdminAccount(reqUser) {
    ...
    if (!account)
        throw fail('Super Admin account not found', 404);
    return account;
}
...
router.post('/login', (req, res, next) => {
    try {
        ...
        if (expectedRole) {
            if (!ALL_ROLES.includes(expectedRole))
                throw fail('Invalid portal role', 400);
...
router.post('/verify-otp', async (req, res, next) => {
    try {
        ...
        if (!account.otp || account.otp !== String(otp))
            throw fail('Invalid OTP', 400);
...
router.post('/oauth/:provider', async (req, res, next) => {
    try {
        ...
        if (!['google', 'apple'].includes(provider))
            throw fail('Unsupported provider');
...
router.post('/impersonate', authenticate, requireSuperAdminOrigin, (req, res, next) => {
    try {
        ...
        if (!role || !ALL_ROLES.includes(role))
            throw fail('Valid role is required', 400);
        ...
        if (!target)
            throw fail(`No account found for role ${role}`, 404);
