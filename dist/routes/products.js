...
router.get('/', optionalAuth, (req, res) => {
    ...
    if (subcategory)
        list = list.filter((p) => p.subcategory.toLowerCase() === String(subcategory).toLowerCase());
    if (brand)
        list = list.filter((p) => p.brand.toLowerCase() === String(brand).toLowerCase());
    if (sellerId)
        list = list.filter((p) => sellerMatches(p.sellerId, String(sellerId)));
    if (featured === 'true')
...
router.post('/:id/reviews', authenticate, async (req, res, next) => {
    try {
        ...
        if (!rating || !body)
            throw fail('rating and body are required');
        const review = {
            id: generateId('rev'),
            userId: req.user.id,
...
router.post('/', authenticate, requireRoles(...adminRoles, 'seller'), async (req, res, next) => {
    try {
        ...
        if (!body.title || !body.price)
            throw fail('title and price are required');
