...
const allowedStatuses = ['draft', 'active', 'paused', 'ended'];
function normalizeStatus(input, fallback) {
    if (typeof input !== 'string')
        return fallback;
    return allowedStatuses.includes(input) ? input : fallback;
}
...
function inActiveSchedule(ad, now = new Date()) {
...
adsAdminRouter.post('/', async (req, res, next) => {
    try {
        ...
        if (!image || typeof image !== 'string')
            throw fail('image is required');
        if (!title || typeof title !== 'string')
            throw fail('title is required');
        if (!link || typeof link !== 'string')
            throw fail('link is required');
        if (!placement || typeof placement !== 'string')
            throw fail('placement is required');
