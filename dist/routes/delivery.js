...
import { saveDelivery, saveDeliveryPartner, saveOrder, } from '../store/persist.js';
import { onDelivered, onFailed, optimizeRoute, reassignDelivery, } from '../lib/deliveryAssignment.js';
import { sendNotification } from '../lib/notifications.js';
import { fail, nowISO, paginate, success, todayISO } from '../utils/helpers.js';
const router = Router();
router.use(authenticate, requireRoles('delivery'));
function getPartner(userId) {
    return db.deliveryPartners.find((p) => p.userId === userId);
}
function requirePartner(userId) {
    const partner = getPartner(userId);
    if (!partner)
        throw fail('Delivery partner profile not found', 404);
    return partner;
}
...
router.patch('/me/availability', async (req, res, next) => {
    try {
        ...
        if (typeof currentLat === 'number')
            partner.currentLat = currentLat;
        if (typeof currentLng === 'number')
            partner.currentLng = currentLng;
...
router.get('/me/dashboard', (req, res, next) => {
    try {
        ...
        const pending = mine.filter((d) => ['assigned', 'accepted', 'picked_up', 'out_for_delivery'].includes(d.status));
        const earningsToday = db.deliveryEarnings.filter((e) => e.deliveryPartnerId === partner.id && isToday(e.createdAt));
        ...
        const completionPct = todayAssigned.length > 0
...
router.get('/me/deliveries', (req, res, next) => {
    try {
        ...
        if (req.query.date === 'today')
            list = list.filter((d) => isToday(d.assignedAt));
...
router.patch('/me/deliveries/:id/accept', async (req, res, next) => {
    try {
        const partner = requirePartner(req.user.id);
        const delivery = db.deliveries.find((d) => d.id === req.params.id && d.deliveryPartnerId === partner.id);
        if (!delivery)
            throw fail('Delivery not found', 404);
        if (delivery.status !== 'assigned')
            throw fail('Delivery cannot be accepted in current status');
...
router.patch('/me/deliveries/:id/reject', async (req, res, next) => {
    try {
        ...
        if (!delivery)
            throw fail('Delivery not found', 404);
        if (delivery.status !== 'assigned')
            throw fail('Only assigned deliveries can be rejected');
...
router.patch('/me/deliveries/:id/status', async (req, res, next) => {
    try {
        ...
        if (status === 'picked_up') {
            ...
            if (photo)
                delivery.proofImageUrl = String(photo);
            const order = db.orders.find((o) => o.id === delivery.orderId);
            if (order) {
            ...
            if (!otpOk && !photoOk)
                throw fail('Delivered status requires OTP verification or photo proof');
            ...
            if (!failureReason)
                throw fail('failureReason is required for failed deliveries');
...
router.get('/me/route', (req, res, next) => {
    try {
        const partner = requirePartner(req.user.id);
        const today = db.deliveries.filter((d) => d.deliveryPartnerId === partner.id &&
...
router.get('/me/cod', (req, res, next) => {
    try {
        const partner = requirePartner(req.user.id);
        const codDeliveries = db.deliveries.filter((d) => d.deliveryPartnerId === partner.id &&
            d.paymentType === 'cod' &&
            isToday(d.assignedAt));
...
router.post('/me/cod/submit', async (req, res, next) => {
    try {
        const partner = requirePartner(req.user.id);
        const ids = Array.isArray(req.body.deliveryIds) ? req.body.deliveryIds : [];
        const toSubmit = db.deliveries.filter((d) => d.deliveryPartnerId === partner.id &&
            d.paymentType === 'cod' &&
            d.status === 'delivered' &&
            !d.codSubmitted &&
...
router.get('/me/earnings', (req, res, next) => {
    try {
        ...
        const list = db.deliveryEarnings
            .filter((e) => e.deliveryPartnerId === partner.id && new Date(e.createdAt).getTime() >= from.getTime())
            .map((e) => {
...
router.get('/me/earnings/statement.csv', (req, res, next) => {
    try {
        ...
        const from = startOfRange(range);
        const list = db.deliveryEarnings.filter((e) => e.deliveryPartnerId === partner.id && new Date(e.createdAt).getTime() >= from.getTime());
