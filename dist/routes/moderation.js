...
import { deleteProduct, saveFlaggedProduct, saveFlaggedReview, saveModerationStats, } from '../store/persist.js';
...
router.get('/reviews/:id', (req, res, next) => {
    try {
        ...
        if (!item)
            throw fail('Flagged review not found', 404);
...
router.post('/reviews/bulk', async (req, res, next) => {
    try {
        ...
        if (!action || !ids?.length)
            throw fail('action and ids required');
...
router.patch('/products/:id', async (req, res, next) => {
    try {
        ...
        if (!item)
            throw fail('Flagged product not found', 404);
        const action = req.body.action;
        if (action === 'approve') {
            item.status = 'approved';
            db.moderationStats.approvedToday += 1;
