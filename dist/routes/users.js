...
import { deleteAddress, deletePaymentMethod, saveAddress, saveAddressesDefaults, saveCustomer, savePaymentMethod, saveUser, } from '../store/persist.js';
...
router.post('/me/addresses', async (req, res, next) => {
    try {
        ...
        if (!body.line1 || !body.city || !body.pincode)
            throw fail('line1, city, pincode required');
...
router.post('/me/payment-methods', async (req, res, next) => {
    try {
        ...
        if (!type || !label)
            throw fail('type and label required');
        ...
        for (const existing of list)
            await savePaymentMethod(existing);
...
router.get('/', requireRoles(...adminRoles), (req, res) => {
    ...
    if (tier)
        list = list.filter((u) => u.tier === tier);
    if (search) {
...
router.patch('/:id', requireRoles(...adminRoles), async (req, res, next) => {
    try {
        ...
        if (action === 'block' || action === 'suspend')
            db.customers[idx].status = 'blocked';
        else if (action === 'unblock')
            db.customers[idx].status = 'active';
        else if (action === 'verify')
            db.customers[idx].isVerified = true;
        else {
            const body = { ...req.body };
