...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!items?.length || !shippingAddress)
            throw fail('items and shippingAddress are required');
        ...
        const orderItems = items.map((item) => {
            ...
            if (!product)
                throw fail(`Product ${item.productId} not found`, 404);
        ...
        if (idemKey)
            idempotencyKeys.set(idemKey, id);
...
router.get('/:id', (req, res, next) => {
    try {
        ...
        if (role === 'seller') {
            const sid = req.user.sellerId || req.user.id;
            if (!order.items.some((i) => sellerMatches(i.sellerId, sid)))
                throw fail('Forbidden', 403);
        }
...
router.post('/:id/return', async (req, res, next) => {
    try {
        ...
        if (order.status !== 'delivered')
            throw fail('Only delivered orders can be returned');
