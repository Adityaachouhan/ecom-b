...
function toleranceFor(tokenLength: number): number {
 ...
 */
export function fuzzySearch<T>(items: T[], query: string, getHaystacks: (item: T) => string[]): T[] {
    ...
    .map((item) => {
      ...
      for (const qt of qTokens) {
        ...
        for (const t of tokens) {
          if (Math.abs(t.length - qt.length) > tolerance + 1) continue
          const dist = levenshtein(qt, t)
          if (dist <= tolerance && dist < bestScore) bestScore = dist
        }
      }
