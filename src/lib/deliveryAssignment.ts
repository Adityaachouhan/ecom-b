import { db, type Delivery, type DeliveryPartner } from '../store/db.js'
import {
  saveDelivery,
  saveDeliveryEarning,
  saveDeliveryPartner,
...
function isSameDay(isoDate: string, day = todayISO()): boolean {
  return isoDate.slice(0, 10) === day.slice(0, 10)
}

function availablePartners(excludeIds: string[] = []): DeliveryPartner[] {
  return db.deliveryPartners.filter(
    (p) =>
...
function buildTrackingForOrder(status: string) {
  const steps = [
...
async function notify(
  userId: string,
  eventType: string,
  data: Record<string, unknown> & { title?: string; body?: string; refId?: string } = {}
) {
...
export function createDeliveryForOrder(
  order: {
    ...
    items?: { sellerId?: string; sellerName?: string }[]
  },
  opts?: { reattemptOf?: string; excludePartnerIds?: string[] }
): Delivery | null {
...
export async function assignNearestPartner(order: {
  ...
  const existing = db.deliveries.find(
    (d) => d.orderId === order.id && d.status !== 'failed' && d.status !== 'delivered'
  )
  if (existing) return existing
  return createDeliveryForOrder(order)
}

export async function reassignDelivery(delivery: Delivery): Promise<Delivery | null> {
  const exclude = [...(delivery.triedPartnerIds || [])]
  if (delivery.deliveryPartnerId) exclude.push(delivery.deliveryPartnerId)

  const partner = nearestPartner(
    delivery.pickupLat ?? DEFAULT_HUB.lat,
    delivery.pickupLng ?? DEFAULT_HUB.lng,
    exclude
  )
...
export async function checkAcceptanceTimeouts() {
  const now = Date.now()
  const timedOut = db.deliveries.filter(
    (d) =>
      d.status === 'assigned' &&
      d.deliveryPartnerId &&
...
function computeEarnings(delivery: Delivery): {
  baseFee: number
  ...
  const dist = haversineKm(
    delivery.pickupLat ?? DEFAULT_HUB.lat,
    delivery.pickupLng ?? DEFAULT_HUB.lng,
    delivery.dropLat ?? DEFAULT_HUB.lat,
    delivery.dropLng ?? DEFAULT_HUB.lng
  )
  const distanceBonus = Math.round(dist * DISTANCE_BONUS_PER_KM)
...
export async function onDelivered(delivery: Delivery) {
  ...
  if (order) {
    ...
    const sellerIds = [...new Set((order.items || []).map((i: { sellerId: string }) => i.sellerId))]
...
export async function onFailed(delivery: Delivery) {
  if (!delivery.deliveryPartnerId) return
  const partner = db.deliveryPartners.find((p) => p.id === delivery.deliveryPartnerId)
  if (!partner) return
  ...
  const failuresToday = db.deliveries.filter(
    (d) =>
      d.deliveryPartnerId === partner.id &&
      d.status === 'failed' &&
      isSameDay(d.deliveredAt || d.assignedAt)
  ).length
  ...
  const returnToSeller = ['damaged_in_transit', 'customer_refused'].includes(
  ...
  if (order) {
    if (returnToSeller) {
      ...
      createDeliveryForOrder(order as never, {
...
export function optimizeRoute(
  ...
  while (remaining.length) {
    remaining.sort(
      (a, b) =>
        haversineKm(lat, lng, a.dropLat ?? DEFAULT_HUB.lat, a.dropLng ?? DEFAULT_HUB.lng) -
...
export function startDeliveryScheduler() {
  setInterval(() => {
    void checkAcceptanceTimeouts().catch((err) =>
      console.error('Delivery acceptance timeout check failed:', err)
    )
  }, 30_000)
}
