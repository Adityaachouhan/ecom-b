import { db, normalizeSellerId, type SellerSettlement } from '../store/db.js'
...
type OrderLike = {
  ...
  orderedAt?: string
  deliveredAt?: string
  items?: Array<{ sellerId: string; price: number; quantity: number }>
}

function commissionRateFor(sellerId: string): number {
...
function hasBlockingReturn(orderId: string, sellerId: string): boolean {
...
export async function ensureSettlementsForOrder(order: OrderLike) {
  ...
  for (const [sellerId, orderAmount] of bySeller) {
    const existing = db.settlements.find(
      (s) => s.orderId === order.id && normalizeSellerId(s.sellerId) === sellerId
    )
...
async function advancePendingToProcessing() {
  ...
  for (const settlement of db.settlements) {
    ...
    if (new Date(deliveredAt).getTime() > cutoffMs) continue
    if (hasBlockingReturn(settlement.orderId, settlement.sellerId)) continue
...
async function processPayoutBatches() {
  ...
  const bySeller = new Map<string, SellerSettlement[]>()
  ...
  for (const [sellerId, rows] of bySeller) {
    ...
    const seller = db.sellers.find((s) => normalizeSellerId(s.id) === sellerId)
    if (seller) {
      seller.pendingPayouts = Math.max(
        0,
        Math.round((Number(seller.pendingPayouts) - totalNet) * 100) / 100
      )
      await saveSeller(seller)
    }

    const sellerAccount = db.accounts.find(
      (a) => (a.sellerId && normalizeSellerId(a.sellerId) === sellerId) || a.id === sellerId
    )
    if (sellerAccount) {
...
export function settlementSummaryForSeller(sellerId: string) {
  const sid = normalizeSellerId(sellerId)
  const list = db.settlements.filter((s) => normalizeSellerId(s.sellerId) === sid)
  const sum = (status: SellerSettlement['status']) =>
    list.filter((s) => s.status === status).reduce((acc, s) => acc + Number(s.netAmount), 0)
