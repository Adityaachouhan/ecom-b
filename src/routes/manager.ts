...
router.get('/delivery-partners', (req, res) => {
  ...
  if (req.query.kycStatus) list = list.filter((p) => p.kycStatus === req.query.kycStatus)
  if (req.query.flagged === 'true') list = list.filter((p) => p.consecutiveFailures >= 3)
  res.json(success(paginate(list, Number(req.query.page), Number(req.query.limit))))
})
...
router.patch('/delivery-partners/:id', async (req, res, next) => {
  try {
    ...
    if (!partner) throw fail('Delivery partner not found', 404)
    const action = req.body.action as string | undefined
    if (action === 'approve') {
