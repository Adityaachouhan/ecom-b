...
router.post('/onboarding', authenticate, async (req, res, next) => {
  try {
    const body = req.body
    if (!body.name || !body.email || !body.gstNumber) throw fail('name, email, gstNumber required')
...
router.get('/me', requireRoles('seller', ...adminRoles), (req, res, next) => {
  try {
    ...
    if (!seller) throw fail('Seller profile not found', 404)
...
router.patch('/me', requireRoles('seller'), async (req, res, next) => {
  try {
    ...
    if (idx === -1) throw fail('Seller not found', 404)
...
router.post('/me/reviews/:id/reply', requireRoles('seller'), (req, res, next) => {
  try {
    ...
    if (!reply) throw fail('reply is required')
...
router.patch('/me/returns/:id', requireRoles('seller'), async (req, res, next) => {
  try {
    ...
    const sid = req.user!.sellerId || req.user!.id
    if (!sellerMatches(ret.sellerId, sid)) throw fail('Forbidden', 403)
    const action = req.body.action || req.body.status
    if (action === 'approve' || action === 'approved') ret.status = 'approved'
    else if (action === 'reject' || action === 'rejected') ret.status = 'rejected'
    else if (action === 'refunded') ret.status = 'refunded'
...
router.post('/me/campaigns', requireRoles('seller'), async (req, res, next) => {
  try {
    ...
    if (!camp.name) throw fail('name is required')
...
router.patch('/:id', requireRoles(...adminRoles, 'manager'), async (req, res, next) => {
  try {
    ...
    if (action === 'approve') db.sellers[idx].status = 'active'
    else if (action === 'suspend') db.sellers[idx].status = 'suspended'
    else db.sellers[idx] = { ...db.sellers[idx], ...req.body, id: db.sellers[idx].id }
    await saveSeller(db.sellers[idx])
