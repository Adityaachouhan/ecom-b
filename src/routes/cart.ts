...
router.post('/items', async (req, res, next) => {
  try {
    ...
    if (!productId) throw fail('productId is required')
    ...
    else cart.push({ productId, quantity: Number(quantity), variantId, size })
...
router.patch('/items/:productId', async (req, res, next) => {
  try {
    ...
    if (!item) throw fail('Cart item not found', 404)
    ...
    if (req.body.variantId !== undefined) item.variantId = req.body.variantId
